...
void useDefensiveItem(Character* character, Item& item)
{
    ...
    if( auto* chDw = dynamic_cast<Dwarf*>(character) )
    {
        chDw->boostArmor(static_cast<int>(item.getBoost() * 1.1));
    }
    else if( auto* chP = dynamic_cast<Paladin*>(character) )
    {
        //same with paladins
        chP->boostArmor(static_cast<int>(item.getBoost() * 1.3 ));
    }
    else if( auto* chDS = dynamic_cast<DragonSlayer*>(character))
    {
        chDS->boostArmor(static_cast<int>(item.getBoost() * 1.5 ));
    }
    else if( auto* chD = dynamic_cast<Dragon*>(character) )
    {
...
void useHelpfulItem(Character* character, Item* item)
{
    if( auto* chDw = dynamic_cast<Dwarf*>(character) )
    {
        chDw->boostHitPoints(static_cast<int>(item->getBoost() * 2));
    }
    else if( auto* chP = dynamic_cast<Paladin*>(character) )
    {
        chP->boostHitPoints(static_cast<int>(item->getBoost() * 1.5));
    }
    else if( auto* chDS = dynamic_cast<DragonSlayer*>(character))
    {
        chDS->boostHitPoints(static_cast<int>(item->getBoost() * 1.25));
    }
    else if( auto* chD = dynamic_cast<Dragon*>(character) )
    {
...
void useAttackItem(Character* character, Item* item)
{
    if( auto* chDw = dynamic_cast<Dwarf*>(character) )
    {
        chDw->boostAttackDamage(static_cast<int>(item->getBoost() * 1.5));
    }
    else if( auto* chP = dynamic_cast<Paladin*>(character) )
    {
        chP->boostAttackDamage(static_cast<int>(item->getBoost() * 1.33));
    }
    else if( auto* chDS = dynamic_cast<DragonSlayer*>(character))
    {
        ...
        chDS->boostAttackDamage(static_cast<int>(character->getAttackDamage() * item->getBoost() * 10));
    }
